...
;(document.querySelector('body') as any).addEventListener(
  'click',
  () => {
...
const SampleLibrary = {
  ...
  setExt: function (newExt: any) {
    var i
    for (i = 0; i <= this.list.length - 1; i++) {
      for (var property in (this as any)[this.list[i]]) {
        ;(this as any)[this.list[i]][property] = (this as any)[this.list[i]][property].replace(
          this.ext,
          newExt
        )
      }
  ...
  load: function (arg: any) {
    ...
    if (Array.isArray(t.instruments)) {
      for (i = 0; i <= t.instruments.length - 1; i++) {
        var newT: any = (this as any)[t.instruments[i]]
        ...
        ;(rt as any)[t.instruments[i]] = new Tone.Sampler(newT, {
      ...
      newT = (this as any)[t.instruments]
